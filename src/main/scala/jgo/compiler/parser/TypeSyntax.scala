...
trait TypeSyntax extends Symbols {
  ...
  lazy val structFieldDecl: P[List[Field]] =       "struct field decl" $
    ( identList ~ goType ~ stringLit.?  ^^ { (ids, t, tag) => ids map { RegularField(_, t, tag) } }
    | "*" ~> typeName    ~ stringLit.?  ^^ { (t, tag)      => EmbeddedField(t.name, t, true,  tag) }
    |        typeName    ~ stringLit.?  ^^ { (t, tag)      => EmbeddedField(t.name, t, false, tag) }
    )
  ...
  lazy val funcTypeResult: P[List[Type]] =      "function-type result" $
    ...
    | funcTypeParams ^^ {
        case (results, isVariadic) =>
          if (isVariadic)
            WithMsg(Error("Result \"parameter\" list may not be variadic"), results)
          else
            results
      }
    )
  
  lazy val funcTypeParams: P[List[Type], Boolean] =
    ...
    "(" ~> repsep(funcParamDecl, ",") <~ ")"  ^^ { //recall that repsep admits zero repetitions
      ...
      val ret = (backwardsParams.reverse, variadic)
      if (err)
        WithMsg(Error("`...' permitted only on the final type in a signature"), ret)
      else
        ret
    }
  ...
  lazy val typeNoAmbigPrefixNoIdent: P_ =
                  "type without an ambiguous prefix or prefixal ident" $
    ( not("<-").named("not `<-'")
